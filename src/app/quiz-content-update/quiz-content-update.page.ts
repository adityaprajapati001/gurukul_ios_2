...
@Component({
  selector: 'app-quiz-content-update',
  templateUrl: './quiz-content-update.page.html',
  styleUrls: ['./quiz-content-update.page.scss'],
})
export class QuizContentUpdatePage implements OnInit {
    ...
    try {
      ...
      this.authService.getAttemptSummary(this.attemptId).subscribe({
        next: (data) => {
          ...
          this.datanew = data.questions[0].html;
  ...
  async startTimer() {
    // // Show loading spinner only once if needed
    // const loading = await this.loadingController.create({
    //   message: 'Loading...',
    //   duration: 1000
    // });
    // await loading.present(); // Show spinner initially if needed
